...
def generate_token(password):
    ...
    with open(password) as pw:
        ...
        my_key = requests.post("https://10.248.2.93:8888/auth/oauth/",
                               data={"grant_type": personDataObj.password, "username": personDataObj.username,
                                     "password": personDataObj.adminPassword, "scope": personDataObj.scope,
                                     "client_secret": personDataObj.client_secret, "client_id": personDataObj.client_id,
                                     "realm": personDataObj.realm}, verify=False)
...
class queryEndPoint(APIView):
    ...
    def get(self, request, id, token):
        ...
        response = requests.post("https://10.248.2.95:8888/api/search/query",
...
def main():
    ...
    parser_query.add_argument("-q", "--query", nargs="?", action="store", type=str,
                              help="Specify search query, e.g. sample name")
    parser_query.add_argument("-i", "--index", nargs="?", action="store", type=str,
                              help="Specify index from HCI to parse")
    parser_query.add_argument("-o", "--output", nargs="?", action="store", type=str,
                              help="Specify file to store outputs")
    parser_query.add_argument("-p", "--password", nargs="?", action="store", type=str,
                              help="Specify file to store outputs")
    ...
    parser_index.add_argument("-i", "--index", nargs="?", action="store", type=str,
                              help="Specify index from HCI to parse, if 'all' list every index and fields")
    parser_index.add_argument("-o", "--output", nargs="?", action="store", type=str,
                              help="Specify file to store outputs")
    parser_index.add_argument("-p", "--password", nargs="?", action="store", type=str,
                              help="Specify file to store outputs")
